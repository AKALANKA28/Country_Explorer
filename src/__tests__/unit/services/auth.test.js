...
describe('Auth Service', () => {
  ...
  describe('registerUser', () => {
    test('returns user data when registration is called', async () => {
      ...
      const user = await registerUser('new@example.com', 'password123', 'New User');
    ...
    test('validates that all fields are required', async () => {
      ...
      await expect(registerUser(null, 'password123', 'New User'))
        .rejects
        .toThrow('All fields are required');
      
      await expect(registerUser('new@example.com', '', 'New User'))
        .rejects
        .toThrow('All fields are required');
      
      await expect(registerUser('new@example.com', 'password123', ''))
        .rejects
        .toThrow('All fields are required');
    });
  });
